...
def date_split(date):
    if "/" in date and date.split("/")[0] not in month_list:
        return date.split("/")
    elif "," in date:
        date = date.replace(",", "")
        return date.split(" ")
...
def date_check(date):
    ...
    try:
        ...
        ValueError
        return False
    for i,j in enumerate(month_list, start=1):
        if x[0] == j:
            x[0] = i
            return x
        elif x[0] == str(i):
...
# print(date_check("October 9, 1701"))
# print(date_split("October 9, 1701"))
# x = ['October', '9', '1701']
# for i,j in enumerate(month_list, start=1):
#         if x[0] == j:
#             x[0] = i
